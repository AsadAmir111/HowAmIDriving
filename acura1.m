function [newTime2,newOutput] = acura1(fileName,dbcName)
    close all;
    ...
    for i = 1:1:addsize;
        Address1(i) = strcmp(Address(i),ID);
    end
...
%         Binary = bitget(newData(i),1:1:32);
        if i == 1
            disp(i);
            disp(newData(i));
            disp(Binary);
    ...
    while 1
        [x,~] = ginput(1);       
        if (x1+50+x2+50)/2 < x && (x1+50+x2+50)/2 < xlimit-90
            tempx = tempx+10;
            x1 = x1+50;
            x2 = x2+50;
